      ...
      <li class="atg_store_orderTotal">
        ...
        <%-- Display order's total. --%>
          ...
          <span class="atg_store_orderSummaryItem">
	       ...
	       <c:if test="${(fn:length(order.paymentGroups) > 1) and (not empty currentStage) and (currentStage == 'billing')}">
			<dsp:getvalueof var="paymentGroupsTotal" vartype="java.lang.Double" value="0.0"/> 
			  <c:forEach var="paymentGroup" varStatus="status" items="${order.paymentGroups}" begin="0">
			  	<dsp:getvalueof var="paymentGroupsTotal" vartype="java.lang.Double" value="${paymentGroupsTotal + paymentGroup.amount}" /> 
			  </c:forEach>
		  </c:if>
      <c:if test="${paymentGroupsTotal < total}">
      	<span class="atg_store_orderSummaryLabel">Remaining Amount<fmt:message key="common.labelSeparator"/></span>
      	<span class="atg_store_orderSummaryItem">
	      	<dsp:include page="/global/gadgets/formattedPrice.jsp">
	              <%-- If there are enough store credits to pay for order, display order's total as 0. --%>
	              <dsp:getvalueof var="remaingAmount" value="${total - paymentGroupsTotal}"/>
	             <dsp:param name="price" value="${remaingAmount > storeCreditAmount ? remaingAmount - storeCreditAmount : 0}"/>
	        </dsp:include>
        </span>
        </c:if>
      </li>
      	<c:if test="${(not empty currentStage) and (currentStage == 'billing')}">
		<dsp:importbean bean="/atg/store/order/purchase/GiftCardBillingFormHandler" />
		<span class="atg_store_orderSummaryItem">
		<div class="form_row" id="gcNumberClass">
					<label for="gift_card_number">Gift Card Number</label>
					 <dsp:input  name="gift_card_number" id="gift_card_number" bean="GiftCardBillingFormHandler.giftCardNumber" autocomplete="OFF"  type="text"/>
		</div>
		</span>
		<div class="form_row" id="gcpinNumberClass">
				<label for="gift_card_pin">Gift Card PIN</label>
				<dsp:input  name="gift_card_pin" id="gift_card_pin" maxlength="4"   bean="GiftCardBillingFormHandler.pinNumber" autocomplete="OFF"  type="text"/>
		</div>
		<dsp:input   value="Apply" bean="GiftCardBillingFormHandler.applyGiftCard"   type="submit"/>
		<dsp:input bean="GiftCardBillingFormHandler.successURL" type="hidden" value="confirm.jsp"/>
		<dsp:input bean="GiftCardBillingFormHandler.errorURL" type="hidden"  value="billing.jsp"/>
		
	</c:if> 
	
	<c:if test="${(fn:length(order.paymentGroups) > 0) and (not empty currentStage) and (currentStage == 'billing')}">
	
		  <c:forEach var="paymentGroup" varStatus="status" items="${order.paymentGroups}" begin="0">
		   <c:if test="${paymentGroup.paymentGroupClassType == 'giftCard'}">
		    <preview:repositoryItem item="${paymentGroup}">
		      <%--div element was added as a wrapper. It makes possible to right click on it in preview--%>
		      <div>
			<dsp:a href=".././billing.jsp" bean="GiftCardBillingFormHandler.removeGiftCardPaymentGroups" value="${paymentGroup.id}"  id="removeGiftCard">
				Remove GiftCard - ${paymentGroup.id}
