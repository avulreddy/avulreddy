...
<dsp:page>
  ...
  <div class="atg_store_checkoutOption" id="atg_store_checkoutOptionArea">
    ...
    <c:if test="${!empty creditCards}">
      ...
      <div class="atg_store_existingCreditCards div_creditCard ${orderPaymentGroupMethod ne 'creditCard' ? ' hide':'' }">
    ...
    <div class="atg_store_addCardInfo div_creditCard ${orderPaymentGroupMethod ne 'creditCard' ? ' hide':'' }">
          ...
          <dsp:getvalueof var="permittedAddresses" vartype="java.lang.Object" param="permittedAddresses"/>
    				...
    				<div id="atg_store_chooseCardAddress" class="${!empty permittedAddresses?'atg_store_existingAddresses ':''}">
						...
						<c:if test="${bmlExist}">	
							...
							<dsp:getvalueof id="orderTotal" bean="/atg/commerce/ShoppingCart.current.priceInfo.total"></dsp:getvalueof>
							<dsp:getvalueof id="merchantType" bean="BMLConfiguration.merchantType"></dsp:getvalueof>
							...
							<dsp:getvalueof id="thresholdPromoAmount" bean="BMLConfiguration.thresholdPromoAmount"></dsp:getvalueof>
							<div style="display: none;">
								<c:if test="${merchantType == -1}">
									<c:if test="${ thresholdPromoAmount <= orderTotal }">
										<dsp:input bean="BMLBillingFormHandler.BMLUserType" type="radio" value="${coreUserStringValue}" checked="true"/>
									 CoreMerchant
									</br>
									<dsp:input bean="BMLBillingFormHandler.BMLUserType" type="radio" value="${promoUserStringValue}"/>
									 PromoMerchant
									</c:if>
									<c:if test="${ thresholdPromoAmount > orderTotal }">
										<dsp:input bean="BMLBillingFormHandler.BMLUserType" type="radio" value="${coreUserStringValue}" checked="true"/>
								...
								<c:if test="${merchantType== 2}">
								   ...
								   <dsp:input bean="BMLBillingFormHandler.BMLUserType" type="radio" value="${promoUserStringValue}" checked="true"/>
									 PromoMerchant
          ...
          <fieldset class="atg_store_billingAddresses">
            ...
            <div class="div_creditCard ${orderPaymentGroupMethod ne 'creditCard'?'hide':''}">
             ...
             <div class="div_billmeLater ${orderPaymentGroupMethod ne 'billmeLater'?'hide':''}">
	            <dsp:include page="billingAddresses.jsp">
	              <dsp:param name="availableBillingAddresses" value="${permittedAddresses}"/>
	              <dsp:param name="formHandler" value="BMLBillingFormHandler"/>
	            </dsp:include>
            </div>
            ...
            <div class="atg_store_saveSelectAddress atg_store_formActions">
              <span class="atg_store_basicButton">
                <div class="div_creditCard ${orderPaymentGroupMethod ne 'creditCard'?'hide':''}">
	               ...
	               <div class="div_billmeLater ${orderPaymentGroupMethod ne 'billmeLater'?'hide':''}">
	               		<dsp:input bean="BMLBillingFormHandler.BMLPaymentSuccessURL"  value="../checkout/bmlPayment.jsp" type="hidden"/> 
							<dsp:input bean="BMLBillingFormHandler.BMLPaymentErrorURL"  value="../checkout/billing.jsp" type="hidden"/>
							<dsp:input type="submit" bean="BMLBillingFormHandler.moveToBMLPayment" value="${continueButtonText}"
		                           alt="${continueButtonText}" id="submit" iclass="atg_store_actionSubmit atg_behavior_disableOnClick"/>
            ...
            <div class="div_creditcard ${orderPaymentGroupMethod ne 'creditCard'?'hide':''}">
			 ...
			 <div class="div_billmeLater ${orderPaymentGroupMethod ne 'billmeLater'?'hide':''}">
				<c:if test="${bmlExist}">					
						<dsp:input bean="BMLBillingFormHandler.BMLPaymentSuccessURL"  value="../checkout/bmlPayment.jsp" type="hidden"/> 
						<dsp:input bean="BMLBillingFormHandler.BMLPaymentErrorURL"  value="../checkout/billing.jsp" type="hidden"/>
					<dsp:input type="submit" bean="BMLBillingFormHandler.moveToBMLPaymentWithNewAddress" value="${reviewOrderButtonText}"
	                       alt="${reviewOrderButtonText}" id="submit" iclass="atg_store_actionSubmit atg_behavior_disableOnClick"/>
				</c:if>            
            </div>
          </span>
          <p>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/Storefront/j2ee/store.war/checkout/gadgets/billingForm.jsp#1 $$Change: 875535 $--%>
