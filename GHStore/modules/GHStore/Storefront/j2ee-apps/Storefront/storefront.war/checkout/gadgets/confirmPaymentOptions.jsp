    ...
    <c:otherwise>
      ...
      <c:forEach var="paymentGroupRelationship" items="${paymentGroupRelationships}">
        ...
        <c:if test="${paymentGroupClassType == 'creditCard'}">
         ...
         <%-- We will display credit cards only, however. --%>
		<c:if test="${paymentGroupClassType == 'paypal'}">
	          <dsp:include page="/checkout/gadgets/paypalPaymentGroupRenderer.jsp">
	            <dsp:param name="isCurrent" param="isCurrent"/>
	            <dsp:param name="paymentGroup" param="paymentGroup"/>
	            <dsp:param name="isExpressCheckout" value="${expressCheckout}"/>
	          </dsp:include>
        </c:if>
	      <c:if test="${paymentGroupClassType == 'giftCard'}">
		          <dsp:include page="/checkout/gadgets/giftCardPaymentGroupRenderer.jsp">
		            <dsp:param name="isCurrent" param="isCurrent"/>
		            <dsp:param name="paymentGroup" param="paymentGroup"/>
		            <dsp:param name="isExpressCheckout" value="${expressCheckout}"/>
		          </dsp:include>
	        </c:if>
	        <c:if test="${paymentGroupClassType == 'billmeLater'}">
	          <dsp:include page="/checkout/gadgets/billmeLaterPaymentGroupRenderer.jsp">
	            <dsp:param name="isCurrent" param="isCurrent"/>
	            <dsp:param name="paymentGroup" param="paymentGroup"/>
	            <dsp:param name="isExpressCheckout" value="${expressCheckout}"/>
	          </dsp:include>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/Storefront/j2ee/store.war/checkout/gadgets/confirmPaymentOptions.jsp#1 $$Change: 875535 $--%>